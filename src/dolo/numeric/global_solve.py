...
def global_solve(model, bounds=None, initial_dr=None, interp_type='smolyak', pert_order=2, T=200, n_s=2, N_e=40, integration='gauss-hermite', integration_orders=[], maxit=500, numdiff=True, polish=True, compiler=None, memory_hungry=True, smolyak_order=3, interp_orders=None, test_solution=False, verbose=False, complementarities=False):
    ...
    dr = time_iteration(sg.grid, sg, xinit, gc.f, gc.g, parms, epsilons, weights, maxit=maxit, nmaxit=50, numdiff=numdiff, verbose=verbose, complementarities=complementarities)
...
def global_solve_new(cmodel, bounds=None, initial_dr=None, interp_type='smolyak', pert_order=2, T=200, n_s=2, N_e=40, integration='gauss-hermite', integration_orders=[], maxit=500, numdiff=True, polish=True, compiler=None, memory_hungry=True, smolyak_order=3, interp_orders=None, test_solution=False, verbose=False):
    ...
    from dolo.compiler.global_solution import time_iteration, stochastic_residuals_2, stochastic_residuals_3
    vprint('Starting time iteration')
    dr = time_iteration(sg.grid, sg, xinit, cm.f, cm.g, parms, epsilons, weights, maxit=maxit, nmaxit=50, numdiff=numdiff, verbose=verbose )
    ...
    if polish: # this will only work with smolyak
        ...
        theta_0 = dr.theta.copy().flatten()
        if not memory_hungry:
            fobj = lambda t: stochastic_residuals_3(dr.grid, t, dr, cm.f, cm.g, parms, epsilons, weights, shape, no_deriv=True)
            dfobj = lambda t: stochastic_residuals_3(dr.grid, t, dr, cm.f, cm.g, parms, epsilons, weights, shape)[1]
        else :
            fobj = lambda t: stochastic_residuals_2(dr.grid, t, dr, cm.f, cm.g, parms, epsilons, weights, shape, no_deriv=True)
            dfobj = lambda t: stochastic_residuals_2(dr.grid, t, dr, cm.f, cm.g, parms, epsilons, weights, shape)[1]

        theta = solver(fobj, theta_0, jac=dfobj, verbose=verbose)
        dr.theta = theta.reshape(shape)

    if test_solution:
        res = stochastic_residuals_2(dr.grid, dr.theta , dr, cm.f, cm.g, parms, epsilons, weights, shape, no_deriv=True)
        if numpy.isfinite(res.flatten()).sum() > 0:
            raise( Exception('Non finite value in residuals.'))
