...
# # CDC Notation and LaTeX derivations for solution of RBC model
#
# This notebook exhibits dolo's solution of a standard Real Business Cycle model, in which a representative agent solves a standard maximization problem:
#
# \begin{eqnarray}
# v(k,a) & = & max_{c} ~~ e^{a} k^{\alpha} {n}^{1-\alpha} + \beta \mathbb{E}v(k',a')
# \end{eqnarray}
#
# with components as described below:
#
# Definitions:
# \begin{eqnarray}
...
# \end{eqnarray}
#
# Where the first of the arbitrage conditions is derived using the marginal utilities of consumption and leisure:
# \begin{eqnarray}
# u^{c} & =  & c^{-\rho} \\ 
# u^{n} & = & - \chi {n}^{\eta}
# \end{eqnarray}
# via the usual condition in which the marginal cost (in utility) and marginal benefit (in wages) 
# of an extra unit of work satisfies the intratemporal optimality condition:
# \begin{eqnarray}
# \left(\frac{W}{1}\right) & = & \left(\frac{\chi {n}^{\eta}}{c^{-\rho}}\right)
# \end{eqnarray}
# which is equivalent to the first 'arbitrage' condition in the model file.
