...
def egm(model, dr0=None, a_grid=None, η_tol=1e-6, maxit=1000, grid=None, dp=None, verbose=False):
    '''
    a_grid: (numpy-array) vector of points used to discretize poststates; must be increasing
    '''

    assert(len(model.symbols['states'])==1)
    assert(len(model.symbols['controls'])==1) # we probably don't need this restriction
    ...
    exo_grid = grid["exo"]
    mdr = DecisionRule(
        exo_grid, endo_grid, dprocess=dp, interp_method='cubic'
    )

    mdr.set_values(np.concatenate([drfut(i,s)[None,:,:] for i in range(n_m)], axis=0))
