...
#         return numpy.linalg.solve(self.jac, rr.ravel()).reshape(rr.shape)
    ...
    def solve_smart(self, rr, tol=1e-10, maxit=1000, verbose=False, filt=None, scale=1.0):
        ...
        grid = self.grid
        fut_S = self.fut_S
        sol, nn = invert_jac(rr*scale,dres,jres,fut_S,n_m,N,n_x,grid,tol=tol,maxit=maxit,verbose=verbose,filt=filt)
        sol /= scale
        return sol
