...
def check_symbols(data):
    ...
    for key,values in cm_symbols.items():
        ...
        if key not in known_symbol_types[model_type]:
            ...
            exc = ModelException("Unknown symbol type '{}' for model type '{}'".format(key, model_type))
            exc.pos = (l0,c0,l1,c1)
            # print(l0,c0,l1,c1)
        ...
        for i,v in enumerate(values):
            ...
            if v in already_declared:
                ...
                exc = ModelException("Symbol '{}' already declared as '{}'. (pos {})".format(v, ll[0], (ll[1][0]+1,ll[1][1])))
                exc.pos = (l0,c0,l1,c1)
                exceptions.append(exc)
            else:
...
def check_equations(data):
    ...
    for eq_type in equations.keys():
        ...
        # should be raised by ruaml.yaml ?
            ...
            exc = ModelException("Equation type {} declared twice at ({})".format(eq_type, pos))
            exc.pos = pos
            exceptions.append(exc)
        else:
        ...
        if specs[eq_type].get('target'):
            for n,eq in enumerate(equations[eq_type]):
                ...
                if lhs_name not in data['symbols'][target]:
                    exc = ModelException("Undeclared assignement target '{}'. Add it to '{}'.".format(lhs_name, target))
                ...
                # if n>len(data['symbols'][target]):
                    ...
                    if lhs_name!=right_name:
                        exc = ModelException("Left hand side should be '{}' instead of '{}'.".format(right_name, lhs_name))
                        exc.pos = [pos[0], pos[1], pos[0], pos[1]+len(lhs_name)]
                        exceptions.append(exc)
        # temp
    return exceptions
...
def check_definitions(data):
    ...
    for k,v in definitions.items():
        ...
        if k in known_symbols:
            exc = ModelException('Symbol {} has already been defined as a model symbol.'.format(k))
            exc.pos = pos
            exceptions.append(exc)
            continue
        if k in new_definitions:
            exc = ModelException('Symbol {} cannot be defined twice.'.format(k))
            exc.pos = pos
            exceptions.append(exc)
            continue
        try:
        ...
        try:
            ...
            for pb in check['problems']:
                ...
                if err_type == 'timing_error':
                    exc = Exception('Timing for variable {} could not be determined.'.format(pb[0]))
                elif  err_type == 'incorrect_timing':
                    exc = Exception('Variable {} cannot have time {}. (Allowed: {})'.format(name, t, pb[4]))
                elif  err_type == 'unknown_function':
                    exc = Exception('Unknown variable/function {}.'.format(name))
                elif  err_type == 'unknown_variable':
                    exc = Exception('Unknown variable/parameter {}.'.format(name))
                else:
                    print(err_type)
                exc.pos = (pos[0], pos[1]+offset, pos[0], pos[1]+offset+len(name))
                exc.type = 'error'
                exceptions.append(exc)
...
def check_calibration(data):
    ...
    for s in all_symbols:
        if s not in calibration.keys():
            ...
            exc = ModelException("Symbol {} has no calibrated value.".format(s))
            exc.pos = pos0
            exc.type = 'warning'
            exceptions.append(exc)
...
def check_all(data):
    ...
    if serious(exceptions): return exceptions
